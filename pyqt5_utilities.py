...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QFileDialog, QLabel, QPushButton,
                             ...
                             QWidget, QDialog, QLineEdit, QSpinBox, QTextEdit, QProgressBar)
import os
import pandas as pd
...
def epoch_selector():
    ...
    def on_value_change():
        ...
        # label.setText('Selected Value: %s' % value)
    ...
    #         error_msg.setText("Epochs must be between 1 and 9999")
    #         error_msg.setStandardButtons(QMessageBox.Ok)
    #         error_msg.exec_()
...
def save_model(parent):
    ...
    def training():
        ...
        fileName, ext = QFileDialog.getSaveFileName(parent, "Save Model As", "", "Keras Models (*.h5);;All Files (*)", options=options)
        if fileName:
            print(ext)
...
def run_nn(parent):
    ...
    def final_message(loss_value):
        ...
        msg.setText('Model saved in %s with loss %.4f' % (parent.save_location, loss_value))
...
def train_and_progress_bar(model, X, y, n_epochs):
    ...
    class TrainingThread(QThread):
        ...
        def train_model(self):
            progress_callback = TrainingProgressCallback(lambda e: self.epoch_begin.emit(e))
